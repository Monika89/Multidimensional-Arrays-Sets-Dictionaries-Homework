...
class MaximalSum
{
    static void Main()
    {
        Console.WriteLine("Please enter the number of the rows (N) and the columns (M), separated by a space: ");
        string[] inputRowsAndColumns = Console.ReadLine().Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries);

        Console.WriteLine("Please enter values for the elements (same number like the number of the columns), separated by a space for each row: ");
